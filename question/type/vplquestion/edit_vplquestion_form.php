...
class qtype_vplquestion_edit_form extends question_edit_form {
    ...
    private function add_fileset_editor($mform, $name, $listname, $editorname) {
        ...
        $mform->addElement('static', $listname, get_string($name, QVPL),
            ...
            <ul id="'.$listname.'" class="filelist"></ul>');
    ...
    public function validation($submitteddata, $files) {
        ...
        if ($submitteddata['validateonsave']) {
            ...
            try {
                ...
                $grade = qtype_vplquestion_extract_fraction($result, $question->templatevpl);
                if ($result->compilation) {
                    $errors['teachercorrection'] = '<pre style="color:inherit">'.htmlspecialchars($result->compilation).'</pre>';
                } else if ($grade !== null) {
                    if ($grade < 1.0) {
                        $errors['teachercorrection'] = '<pre style="color:inherit">'.htmlspecialchars($result->evaluation).'</pre>';
                    }
                } else {
                    $errors['teachercorrection'] = get_string('nogradeerror', QVPL);
                }
            } catch (Exception $e) {
                $errors['teachercorrection'] = get_string('nogradeerror', QVPL);
            }
