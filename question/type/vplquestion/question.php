...
class qtype_vplquestion_question extends question_graded_automatically {
    ...
    public function grade_response(array $response) {
        ...
        $grade = qtype_vplquestion_extract_fraction($result, $this->templatevpl);
        ...
        if ($currentrecord === false) {
            $newrecord = array_merge($params, array('value' => json_encode($result)));
            $DB->insert_record($table, $newrecord, false);
        } else {
            $currentrecord->value = json_encode($result);
