...
'qtype_vplquestion/scriptsloader'], function($, jqui, log, VPLService, CodeEditors, ScriptsLoader) {
    ...
    function setupExecfiles(execfiles, keepContents, fileTabs, placeholder, $hiddenField) {
        ...
        execfiles.forEach(function(execfile) {
            ...
            $fileTabs.append('<li class="execfilename">' +
                                 '<a ' + (content.startsWith('UNUSED') ? 'class="unused"' : '') + ' href="#">' +
                                     execfile.name +
                                 '</a>' +
                             '</li>');
        });
        ...
        $(fileTabs + ' .execfilename a').click(function(event) {
        ...
        updateExecfile(null, $(fileTabs + ' .execfilename a').first(), aceEditor, $hiddenField);
    }
    ...
    function setupTemplateChangeManager(helpButton) {
        ...
        var $templateChangeDialog = $('<div>' + templateChangeMessage + '</div>').dialog({
            autoOpen: false,
            dialogClass: 'vplchangedialog',
